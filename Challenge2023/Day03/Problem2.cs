...
namespace Challenge2023.Day03
{
    internal class Problem2 : ProblemBase
    {
        ...
        readonly Dictionary<Point, (int partNumber, Symbol achorSymbol)> SymbolAdjacentDigits = [];
        ...
        {
            var gearDigits = SymbolAdjacentDigits.Values.Where(x => x.achorSymbol.Value == '*')
                                                        .GroupBy(x => x.achorSymbol.Point)
                                                        .Where(x => x.Count() == 2)
                                                        .Select(x =>
                                                        {
                                                            var ratio = 1;
                                                            foreach (var (partNumber, achorSymbol) in x)
                                                            {
                                                                ratio *= partNumber;
                                                            }
                                                            return ratio;
